<template>
    <div>
        <div class="text-xs font-bold uppercase tracking-widest text-indigo-400 px-4 py-2 mb-2">
            {{ emoji }} {{ title }}
        </div>
        <div class="space-y-3">
            <button
                ...
                type="button"
                :class="{
                    'bg-indigo-600 border border-indigo-500 shadow-lg':
                        wizard.currentStep === step.id,
                    'bg-indigo-700/50 border border-indigo-600':
                        wizard.currentStep !== step.id &&
                        (step.id <= wizard.currentStep || wizard.assessmentComplete),
                    'hover:bg-indigo-600/70':
                        step.id <= wizard.currentStep || wizard.assessmentComplete,
                    'opacity-50 cursor-not-allowed bg-indigo-900/30 border border-indigo-700':
                        step.id > wizard.currentStep && !wizard.assessmentComplete,
